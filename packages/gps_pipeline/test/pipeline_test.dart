...
void main() {
  group('GpsPipeline Tests', () {
    test('Stationary point -> suppressed', () {
      ...
      var res = pipeline.process(RawFix(
          lat: 0,
          lng: 0,
          elevation: 0,
          timestamp: t0,
      ...
      // Should be stationary now (which emits rejected to suppress drift)
      expect(res?.filterStatus, FilterStatus.rejected);
      expect(res?.rejectReason, RejectReason.stationary);
    });
    ...
    test('Accuracy > 30m -> rejected', () {
      ...
      final res = pipeline.process(RawFix(
          ...
          accuracy: 50));
      expect(res?.filterStatus, FilterStatus.rejected);
      expect(res?.rejectReason, RejectReason.poorAccuracy);
    });
    ...
    test('Mocked fix -> rejected', () {
      ...
      final res = pipeline.process(RawFix(
          ...
          isMocked: true));
      expect(res?.filterStatus, FilterStatus.rejected);
      expect(res?.rejectReason, RejectReason.mocked);
    });
    ...
    test('3-point spike -> caught', () {
      ...
      expect(res?.filterStatus, FilterStatus.rejected);
      expect(res?.rejectReason, RejectReason.spikeDetected);
    });
  });
}
